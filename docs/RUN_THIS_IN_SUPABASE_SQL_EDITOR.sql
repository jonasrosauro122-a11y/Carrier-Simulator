...
-- Scenario tracking columns used by the ordered Home/Auto simulator.
alter table carrier_quote_sessions add column if not exists scenario_id text;
alter table carrier_quote_sessions add column if not exists scenario_title text;
alter table carrier_quote_sessions add column if not exists scenario_order integer default 0;
alter table carrier_quotes add column if not exists scenario_id text;
alter table carrier_quotes add column if not exists scenario_title text;
alter table carrier_quotes add column if not exists scenario_order integer default 0;

create index if not exists idx_carrier_quote_sessions_scenario on carrier_quote_sessions(scenario_id);
create index if not exists idx_carrier_quotes_scenario on carrier_quotes(scenario_id);
create index if not exists idx_carrier_quotes_va_email on carrier_quotes(va_email);
create index if not exists idx_carrier_quotes_created on carrier_quotes(created_at desc);


-- =====================================================================
-- BEGIN NORMALIZED NEW BUSINESS SCHEMA + FOUR ORDERED SCENARIO SEEDS
-- This keeps the database ready for future Supabase Auth / normalized reporting.
-- The current static app writes live attempts into carrier_* tables above.
-- =====================================================================
